...
def eval_class(gt_annos,
    ...
    Args:
        gt_annos (dict): ground truth annotations.
        dt_annos (dict): detection annotations.
    ...
    for m, current_class in enumerate(current_classes):
        #print('Stats for {}/{}'.format(ID2CLASSNAME[current_class], ID2METRIC[metric]))
        for idx_l, difficulty in enumerate(difficultys):
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for i in range(len(gt_annos)):
                    ...
                    #print(gt_annos[i]['filename'], tp, fp, fn)
                    total_tp += tp
                    total_fn += fn
                    total_fp += fp
...
def get_mAP(prec):
    sums = 0
    ## the block below is from mmdetection3d,
    ## which gives slightly higher score and 
    ## differs from the original KITTI benchmark.
    #for i in range(0, prec.shape[-1], 4):
    #    sums = sums + prec[..., i]
    #return sums / 11 * 100
    
    ## Modifiying according to the reference in the original KITTI benchmark:
    ##       https://arxiv.org/abs/1905.12365
    total_recall_points = prec.shape[-1] 
    for i in range(1, total_recall_points):
        sums = sums + prec[..., i]
    return sums / (total_recall_points-1) * 100
...
def kitti_eval(gt_annos,
    ...
    overlap_0_7 = np.array([[0.7, 0.5, 0.5, 0.7, 0.7, 0.5, 0.7], 
                            [0.7, 0.5, 0.5, 0.7, 0.7, 0.5, 0.7],
                            [0.7, 0.5, 0.5, 0.7, 0.7, 0.5, 0.7]])
    
    #CANCEL THE CALCULATION OF LOOSER OVERLAPS
    #overlap_0_5 = np.array([[0.7, 0.5, 0.5, 0.7, 0.7, 0.5, 0.7],
    #                        [0.5, 0.25, 0.25, 0.5, 0.5, 0.25, 0.5],
    #                        [0.5, 0.25, 0.25, 0.5, 0.5, 0.25, 0.5]])
    #min_overlaps = np.stack([overlap_0_7, overlap_0_5], axis=0)  # dims: [2, 3, 7]
    
    min_overlaps = overlap_0_7[np.newaxis, :] # dims [1,3,7]
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            if mAPbbox is not None:
                result += '  bbox AP:{:.2f}, {:.2f}, {:.2f}\n'.format(
                    *mAPbbox[j, :, i])
            if mAPbev is not None:
                result += '  bev  AP:{:.2f}, {:.2f}, {:.2f}\n'.format(
                    *mAPbev[j, :, i])
            if mAP3d is not None:
                result += '  3d   AP:{:.2f}, {:.2f}, {:.2f}\n'.format(
                    *mAP3d[j, :, i])
            ...
            for idx in range(3):
                # DONT USE LOOSER OVERLAPS (i>0)
                #if i == 0:
                #    postfix = f'{difficulty[idx]}_strict'
                #else:
                #    postfix = f'{difficulty[idx]}_loose'
                # we have single (strict) overlap values complying with original KITTI bencmark
                postfix = f'{difficulty[idx]}'
                prefix = f'{curcls_name}'
                if mAP3d is not None:
                    ret_dict[f'{prefix}_3d_{postfix}'] = mAP3d[j, idx, i]
                if mAPbev is not None:
                    ret_dict[f'{prefix}_bev_{postfix}'] = mAPbev[j, idx, i]
                if mAPbbox is not None:
                    ret_dict[f'{prefix}_bbox_{postfix}'] = mAPbbox[j, idx, i]
                if mAPaos is not None:
                    ret_dict[f'{prefix}_aos_{postfix}'] = mAPbbox[j, idx, i]
    ...
    if len(current_classes) > 1:
        ...
        for idx in range(3):
            postfix = f'{difficulty[idx]}'
            if mAP3d is not None:
                ret_dict[f'Overall_3d_{postfix}'] = mAP3d[idx, 0]
            if mAPbev is not None:
                ret_dict[f'Overall_bev_{postfix}'] = mAPbev[idx, 0]
            if mAPbbox is not None:
                ret_dict[f'Overall_bbox_{postfix}'] = mAPbbox[idx, 0]
            if mAPaos is not None:
                ret_dict[f'Overall_aos_{postfix}'] = mAPbbox[idx, 0]
